...
class Figure:
    ...
    def calc_abs_pos(self):
        scaledVertex1 = Figure.scaleVertex(self._vertex1, self.scale)
        scaledVertex2 = Figure.scaleVertex(self._vertex2, self.scale)
        M = self.rotationMatrix()
        turnedVertex1 = Figure.multMatrixVector(M, scaledVertex1)
        turnedVertex2 = Figure.multMatrixVector(M, scaledVertex2)
    ...
    def draw(self):
        ...
        turtle.setpos(*self.position)
        turtle.up()
        turtle.color(Figure.default_color)



if __name__ == '__main__':
    turtle.speed(0)
    triangle = Figure(100, 100, 100, 0)
    triangle.draw()
    # turtle.clear()
